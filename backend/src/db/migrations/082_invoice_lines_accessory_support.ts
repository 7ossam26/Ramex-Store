...
export async function up(db: Knex): Promise<void> {
  await db.schema.alterTable('invoice_lines', (t) => {
    // Make roll_id nullable so accessory lines can omit it
  ...
  await db.raw(`
    ...
    ADD CONSTRAINT chk_invoice_lines_item_type CHECK (
...
export async function down(db: Knex): Promise<void> {
  await db.raw(`ALTER TABLE invoice_lines DROP CONSTRAINT IF EXISTS chk_invoice_lines_item_type`);

...
export async function listStockMovements(
  ...
  if (filters.search !== undefined) {
    ...
    base.where((qb) => {
      qb.whereILike('r.internal_barcode', term).orWhereILike('f.name_ar', term);
    });
  }
  if (filters.fabric_id !== undefined) base.where('r.fabric_id', filters.fabric_id);
  if (filters.color_id !== undefined) base.where('r.color_id', filters.color_id);

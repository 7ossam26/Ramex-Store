...
>
> **Amendment 2026-06-07 (Permissions Hardening — Phase 5 complete)**: The permissions system is now fully enforced end-to-end. (1) `cash_drawer.approve` action added — gates admin-level finance operations (opening balance, owner withdrawal, bank CRUD, treasuries overview, expense approval/rejection) so `shop_seller` (who has `cash_drawer.write`) cannot reach them. Migration 073 seeds the new rows. (2) All 7 previously `requireRole('owner','super_admin')` finance admin routes migrated to `requirePermission('cash_drawer','approve')`. (3) Frontend: every route in `App.tsx` is wrapped in `<PermGate>`; unauthorised direct URL → `/no-access` (Arabic 403 page); nav items for settings/hr/pos/treasury.overview are now permission-driven rather than role-hardcoded. (4) CI gate: `backend/scripts/audit-routes.ts` now exits with code 1 if any route has no guard — 0 NONE routes as of this amendment. (5) `docs/permissions.md` added. Matrix coverage: 121/163 routes matrix-enforced; remaining 42 are intentional `requireRole('super_admin')`, inline `can()`, or pre-auth endpoints. See `docs/permissions.md` for full reference.

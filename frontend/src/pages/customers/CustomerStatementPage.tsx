...
import {
  customersApi,
  customerStatementExportUrl,
  type CustomerStatement,
  type StatementRow,
  type StatementVariant,
  type StatementExportFormat,
} from '@/lib/customers-api';
import { EGP, fmtMoney } from '@/components/dashboard/format';
import { Button } from '@/components/ui/button';
import { Skeleton } from '@/components/Skeleton';
import { ErrorBanner } from '@/components/ErrorBanner';
import { cn } from '@/lib/utils';

const st = ar.customers.statement;
...
/** Money with the EGP symbol; the customer side is single-currency. */
function money(n: number): string {
  return `${fmtMoney(n)} ${EGP}`;
}
...
export function CustomerStatementPage() {
  ...
  const stmtQ = useQuery<CustomerStatement>({
    queryKey: ['customer-statement', idNum, from, to],
    queryFn: () => customersApi.getStatement(idNum, from, to),
  ...
  const exportUrl = (format: StatementExportFormat) =>
    customerStatementExportUrl(idNum, { from, to, variant, format });
...
function StatementDocument({ data, variant }: { data: CustomerStatement; variant: StatementVariant }) {
      ...
      <table className="w-full border-collapse text-sm">
        ...
        <tbody>
          ...
          {data.rows.length === 0 ? (
            ...
            data.rows.map((r, i) => (
              <StatementRows key={i} row={r} detailed={variant === 'detailed'} />
            ))
          )}
          ...
          <tr className="bg-neutral-100 font-bold" style={{ pageBreakInside: 'avoid' }}>
            ...
            <td className="border border-neutral-200 px-2 py-2 text-end">
              <Num>{money(data.totalCredit)}</Num>
            </td>
            <td className="border border-neutral-200 px-2 py-2 text-end">
...
function StatementRows({ row, detailed }: { row: StatementRow; detailed: boolean }) {
  return (
    <>
      <tr style={{ pageBreakInside: 'avoid' }}>
        ...
        <td className="border border-neutral-200 px-2 py-1.5 text-end">
          {row.credit ? <Num>{money(row.credit)}</Num> : ''}
        </td>

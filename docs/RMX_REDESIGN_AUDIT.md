...
## 18. Closeout — Phase 7 (2026-05-12)

The `rmx-new-ui` redesign cycle is complete. This section records the final
state of stack additions, tokens, decisions, and known follow-ups so future
work has a single landing spot before opening the codebase.

### 18.1 Final stack additions

| Concern | Choice | Where |
|---|---|---|
| Motion library | `framer-motion` `^11.18.2` | Phase 2 added. Used across shell, hub landings, dashboard, tabs, charts. |
| Chart library | `recharts` `^3.8.1` | Phase 5 added. Themed via [`pages/reports/SecondaryReportChart.tsx`](../frontend/src/pages/reports/SecondaryReportChart.tsx) CHART_PALETTE constant. Lazy-loaded on Reports routes in Phase 7. |
| Fonts | `@fontsource/ibm-plex-sans-arabic` + `@fontsource/ibm-plex-sans` (woff2, self-hosted via npm) | Phase 1. Weights 300/400/500/600/700 for Arabic, 400/500/600/700 for Latin. |
| Removed | `zustand` (unused), `@radix-ui/react-toast` (replaced by inline Toast primitive) | Phase 7 dead-code sweep. |

### 18.2 Final palette — Warm Editorial

Picked in Phase 1, 2026-05-12. Boutique-editorial register: warm-white surface, deep midnight navy chrome, terracotta accent, sage success. Full hex list in [`design-system/MASTER.md`](../../design-system/MASTER.md) §2.

| Token | Hex |
|---|---|
| `surface` | `#FBF7F2` |
| `chrome` | `#16213A` |
| `accent` | `#C4624A` (terracotta) |
| `success` | `#7C9472` (sage) |
| `warning` | `#D89B3A` (amber) |
| `danger` | `#B83B3B` (warm red) |
| `info` | `#4F6B8E` (muted blue) |

### 18.3 Token system — final shape

**Location:** [`frontend/src/index.css`](../frontend/src/index.css) (HSL CSS variables) → [`frontend/tailwind.config.ts`](../frontend/tailwind.config.ts) (color/scale extensions reading those variables).
...
| Motion duration | 75 / 150 / 200 / 300 / 500 ms | tailwind.config |
| Motion easing | standard / decelerate / accelerate / emphasized | tailwind.config |
| Z-index | base / dropdown / sticky / overlay / modal / popover / toast | tailwind.config |
| Typography scale | xs–5xl with explicit line-heights | tailwind.config |
| Font weights | light/normal/medium/semibold/bold | tailwind.config |

**Print scope:** index.css §"Print styles" overrides surface/text/border tokens to white-on-black inside `@media print`, hides `header[role='banner']`, `aside[role='navigation']`, `[data-print='hide']`, and unflagged `[role='dialog'][aria-modal='true']`. Codes pinned to IBM Plex Mono via `.rmx-print-code`.

**Motion scope:**
- `[data-motion="reduced"]` opt-in on a subtree (POS root wraps in this) — collapses durations to 1ms, removes transform motion.
- `@media (prefers-reduced-motion: reduce)` globally does the same.
- `data-functional-motion` attribute escapes the explicit `data-motion="reduced"` scope (POS success-flash, error-shake, focus tween) but still yields to the OS preference.

### 18.4 Bundle shape — Phase 7 build

`npm run build -w frontend` produces (gzipped):

| Chunk | Size (gz) | Loaded when |
|---|---|---|
| `react` | 53 kb | initial |
| `motion` (framer-motion) | 38 kb | initial |
| `radix` (Dialog + DropdownMenu + Label + Slot) | 29 kb | initial |
| `query` (react-query + axios) | 29 kb | initial |
| `forms` (react-hook-form + zod) | 22 kb | initial |
| `icons` (lucide-react) | 7 kb | initial |
| `index` (app code + non-lazy pages) | ~115 kb | initial |
| `SecondaryReport` (incl. Recharts) | 120 kb | only on `/reports/*` |
| `DailyReport` | 2 kb | only on `/reports/daily` |
| Fonts (woff2 subsets, all weights) | ~20–46 kb each, fetched on demand via `unicode-range` | font cache |

**Lazy boundaries:** `pages/reports/DailyReport` and `pages/reports/SecondaryReport` are `React.lazy` in `App.tsx` behind a single `<Suspense fallback={<RouteFallback />} />` boundary. Recharts is auto-isolated into the SecondaryReport chunk by Rollup. Everyone who never opens a sub-report saves ~120 kb gz.

### 18.5 Known follow-ups (intentionally deferred)

1. **Font preload hints** — woff2 assets are bundled with hashed names by Vite, so a static `<link rel="preload">` in `index.html` would go stale every build. The right fix is `vite-plugin-preload` or a small plugin that emits preload tags from the manifest. For now `@fontsource/*` ships `font-display: swap` so first paint is unaffected.
2. **ESLint config** — `eslint.config.js` was not generated; `npm run lint` falls back to whatever ESLint can discover. `typecheck` is the gate that matters; lint is a follow-up.
...
- Verint-style RTL right-side rail + flyout with click-only triggers, ⌘K command palette, RTL-aware mobile slide-over.
- Warm Editorial palette + IBM Plex Sans Arabic/Latin + complete Re-Skin Standard token set.
- Re-skinned: Dashboard, POS (motion-reduced), Items, Inventory + sub-pages, Orders, Customers, Invoices/Returns, Treasury + sub-pages, Reports + 10 sub-reports, Approvals, Settings (3-column island preserved).
- Shared primitives: PageHeader, FormField, StatusPill, FilterChip, TableFilterBar, TableSkeleton, EmptyState, ErrorBanner, ResponsiveDialog, ResponsiveTable, Toast, Tooltip, Badge, Skeleton.
- `@media print` produces clean snapshots of InvoiceDetail and DailyReport. POS receipt and invoice PDFs remain backend-rendered.
- Lazy-loaded chart routes; vendor bundle split into stable, cacheable chunks.
- `prefers-reduced-motion` honored globally; POS additionally wrapped in `data-motion="reduced"` with functional-motion escapes.
